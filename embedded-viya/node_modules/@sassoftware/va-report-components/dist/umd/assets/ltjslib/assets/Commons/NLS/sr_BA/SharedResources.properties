# CLDR version 21.0.1
# defect S1326950

# I18NOK:FileNotLocalizable
#
# sr_BA/SharedResources.properties
#
 
# DayNames
dayNames=недеља,понедељак,уторак,сриједа,четвртак,петак,субота
dayNamesShort=нед,пон,уто,сри,чет,пет,суб
dayNamesShortest=н,п,у,с,ч,п,с
firstDayOfWeek=1
 
# Stand-Alone DayNames
SAdayNames=недеља,понедељак,уторак,среда,четвртак,петак,субота
SAdayNamesShort=нед,пон,уто,сре,чет,пет,суб
SAdayNamesShortest=н,п,у,с,ч,п,с
 
# MonthNames
monthNames=јануар,фебруар,март,април,мај,јуни,јули,август,септембар,октобар,новембар,децембар
monthNamesShort=јан,феб,мар,апр,мај,јун,јул,авг,сеп,окт,нов,дец
monthNamesShortest=ј,ф,м,а,м,ј,ј,а,с,о,н,д
 
# Stand-Alone MonthNames
SAmonthNames=јануар,фебруар,март,април,мај,јун,јул,август,септембар,октобар,новембар,децембар
SAmonthNamesShort=јан,феб,мар,апр,мај,јун,јул,авг,сеп,окт,нов,дец
SAmonthNamesShortest=ј,ф,м,а,м,ј,ј,а,с,о,н,д
 
# QuarterNames
quarterNames=Прво тромесечје,Друго тромесечје,Треће тромесечје,Четврто тромесечје
quarterNamesShort=К1,К2,К3,К4
quarterNamesShortest=1,2,3,4
 
# Stand-Alone QuarterNames
SAquarterNames=Прво тромесечје,Друго тромесечје,Треће тромесечје,Четврто тромесечје
SAquarterNamesShort=К1,К2,К3,К4
SAquarterNamesShortest=1,2,3,4
 
# CurrencyFormatter, NumberFormatter, Slider
decimalSeparatorFrom=,
decimalSeparatorTo=,
thousandsSeparatorFrom=.
thousandsSeparatorTo=.
 
# CurrencyFormatter, CurrencyValidator
alignSymbol=right
currencySymbol=КМ.
 
# alignNumberNegativeSign
alignNumberNegativeSign=left
 
# DateBase
am=пре подне
pm=поподне
 
eraADNameShortest=н.е.
eraADNameShort=н. е.
eraADNameLong=Нове ере
 
eraBCNameShortest=п.н.е.
eraBCNameShort=п. н. е.
eraBCNameLong=Пре нове ере
 
# Text Direction: supported values are LTR and RTL
textDirection=LTR

# compressed number format
compactDecimalShort.1000=0 hilj.
compactDecimalShort.10000=00 hilj.
compactDecimalShort.100000=000 hilj.
compactDecimalShort.1000000=0 mil.
compactDecimalShort.10000000=00 mil.
compactDecimalShort.100000000=000 mil.
compactDecimalShort.1000000000=0 mlrd.
compactDecimalShort.10000000000=00 mlrd.
compactDecimalShort.100000000000=000 mlrd.
compactDecimalShort.1000000000000=0 bil.
compactDecimalShort.10000000000000=00 bil.
compactDecimalShort.100000000000000=000 bil.
